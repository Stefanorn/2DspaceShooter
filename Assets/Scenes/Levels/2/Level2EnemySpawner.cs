...
public class Level2EnemySpawner : MonoBehaviour {
	...
	void Update () {
		...
		if (nextEnemy <= 0) {
			...
			if(enemyCounter <= -15){
				Instantiate(enemys[Random.Range(0,1)], transform.position + offset, Quaternion.identity);
			}		
			if(enemyCounter <= -30){
				Instantiate(enemys[Random.Range(0,enemys.Length)], transform.position + offset, Quaternion.identity);
			}
			if(enemyCounter == -70){
				EndLevelwave();
			}
